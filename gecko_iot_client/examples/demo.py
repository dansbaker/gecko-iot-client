    ...
    for zone_type, zone_list in zones_dict.items():
        ...
        for zone in zone_list:
            ...
            if zone_type == ZoneType.FLOW_ZONE:
                ...
                print(f"     Flow State: Active={state['active']}, Speed={state['speed']}%")
            elif zone_type == ZoneType.LIGHTING_ZONE:
                state = zone.get_lighting_state()
                color = state.get('color')
                if color:
                    print(f"     Light State: Active={state['active']}, Color=RGB({color['r']},{color['g']},{color['b']})")
                else:
                    print(f"     Light State: Active={state['active']}, No Color Set")
            elif zone_type == ZoneType.TEMPERATURE_CONTROL_ZONE:
                print(f"     Temp State: Current={zone.temperature}°C, Target={zone.target_temperature}°C, Status={zone.status}")
    print("=" * 50)
    ...
    with client:
        ...
        while True:
            try:
                ...
                if lighting_zones:
                    light_zone = lighting_zones[0]  # Get first lighting zone - we know it's a LightingZone
                    toggle_counter += 1
